 ...
 */
class TwoStringPlotter : AutoCloseable, CoroutineScope {
    ...
    fun fastDraw(points: List<NormalVector2D>) = runBlocking {
        ...
        points.forEachIndexed { idx, targetLocation ->
            ...
            do {
                ...
                val shortTarget = location.add(shortMove).asNormalVector2D()
    ...
    private fun asyncMoveToLocation(target: NormalVector2D) {
        ...
        val deltaNormalLeft = abs(targetNormalLenLeft - currentNormalLenLeft)
        val deltaNormalRight = abs(targetNormalLenRight - currentNormalLenRight)
    ...
    override fun toString(): String =
        "Plotter state: {loc:$location, leftT:${spoolLeft.tachoCount}, rightT:${spoolRight.tachoCount}} "
    ...
    companion object {
        ...
        fun xyToHypot(target: Vector2D): Pair<Double, Double> {
            val normalTarget = target.asNormalVector2D()
            val hypotenuseLeft = sqrt(normalTarget.x * normalTarget.x + normalTarget.y * normalTarget.y)
            val xb = 1.0 - normalTarget.x  // same as V-M2 in the picture
            val hypotenuseRight = sqrt(xb * xb + normalTarget.y * normalTarget.y)
        ...
        fun hypotToXY(hypotenuseLeft: Double, hypotenuseRight: Double): NormalVector2D {
            ...
            val y = sqrt(s * (s - topEdge) * (s - hypotenuseLeft) * (s - hypotenuseRight)) / (0.5 * topEdge)
            val x = sqrt(hypotenuseLeft * hypotenuseLeft - y * y)
